...
use WechatApi\src\wechatPay\lib\WxPayReverse;
...
class MicroPay
{
    ...
    /**
     * @var WxPayMicroPay null
     */
    public $wxMicroPay = null;
     ...
     * @return $this
     */
    public function setWxMicroPay($wxMicroPay)
    {
        $this->wxMicroPay = $wxMicroPay;
        return $this;
    }
    ...
    {
        //①、提交被扫支付
        $result          = WxPayApi::micropay($this->wxPayConfig, $this->wxMicroPay, 5);
        $this->payResult = $result;
        //如果返回成功
        if (!array_key_exists("return_code", $result)
            || !array_key_exists("result_code", $result)) {
            $this->errorMessage = "接口调用失败，请确认是否输入是否有误！";
        }

        //取订单号
        $out_trade_no = $this->wxMicroPay->GetOut_trade_no();
    ...
    public function query($out_trade_no, &$succCode)
    {
        ...
        try {
            $result = WxPayApi::orderQuery($this->wxPayConfig, $queryOrderInput);
        } catch (\Exception $e) {
    ...
    public function cancel($out_trade_no, $depth = 0)
    {
        try {
            ...
            $result = WxPayApi::reverse($this->wxPayConfig, $clostOrder);
            //接口调用失败
            if ($result["return_code"] != "SUCCESS") {
                return false;
            }
